...
ELSE Bookings.slots * 2 * Facilities.membercost
END AS cost
FROM Bookings
INNER JOIN Facilities ON Bookings.facid = Facilities.facid
AND Bookings.starttime LIKE '2012-09-14%'
AND ((Bookings.memid = 0) AND (Facilities.guestcost * Bookings.slots * 2 > 30))
OR ((Bookings.memid != 0) AND (Facilities.membercost * Bookings.slots * 2 > 30))
INNER JOIN Members ON Bookings.memid = Members.memid
ORDER BY cost DESC
...
SELECT month, name
FROM (
		SELECT f.name AS name, date_trunc('month', starttime) AS month
		FROM Bookings AS b
		INNER JOIN Facilities AS f
		ON b.facid = f.facid
		GROUP BY f.facid, month) AS sub
ORDER BY name, month;
